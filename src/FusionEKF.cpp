...
FusionEKF::FusionEKF() {
  ...
  //Info: R matrices values provided by Udacity for Laser and Radar
  R_laser_ << 0.0225, 0,
              0, 0.0225;
  ...
  //Info: Since the LIDAR only measure position: the H matrix is projected from 4D state vector to 2D. 
  H_laser_ << 1, 0, 0, 0,
      0, 1, 0, 0;
...
void FusionEKF::ProcessMeasurement(const MeasurementPackage &measurement_pack) {
  ...
  float dt = (measurement_pack.timestamp_ - previous_timestamp_) / 1000000.0; //time in seconds
  previous_timestamp_ = measurement_pack.timestamp_; //updating the timestamp
  ekf_.F_(0, 2) = dt;
  ekf_.F_(1, 3) = dt;

  //updating the process noise co-variance matrix
  float noise_ax = 9;
  float noise_ay = 9;
